...
<script type="text/javascript">
	...
	$(function(){
		...
		$("input[name=loginname]").blur(function(){
			...
			$.post("${pageContext.request.contextPath}/user_ifName.action","uname="+loginname,function(data){
				if(data!=0){
					$("input[name=loginname]").css({"border":"2px solid #0F0"});
					$("#uid").val(data);
					p1 = true;
				}else{
// 					alert($("input[name=loginname]").val()+"登录名已存在!");
					$("input[name=loginname]").css({"border":"2px solid red"});
					$("input[name=loginname]").next.css("color","red").text("用户名以存在!");
				}
			});
		});
...
<body>
	...
	<!--显示表单内容-->
	<div id=MainArea>
		<form action="user_userAdd.action?uid=0" method="post">
					...
					<table cellpadding="0" cellspacing="0" class="mainForm">
						...
						<tr>
							...
							<td><input type="text" name="loginname"
								value="${olduser.loginname }" class="InputStyle" /><span>（登录名要唯一）</span></td>
						</tr>
						<tr>
							<td>姓名</td>
