...
<script language="javascript" src="${pageContext.request.contextPath}/script/jquery.js"></script>
    ...
    function gotoPageNum(num,uid){
    	...
    	location.href ="email_box.action?uid="+uid+"&startnum="+num+"&box=waste";
    }
    function goNum(uid){
    	var a = $(".inputStyle").val();
    	//alert("email_box.action?uid="+uid+"&startnum="+a+"&box=waste");
    	location.href ="email_box.action?uid="+uid+"&startnum="+a+"&box=waste";
    }
    </script>
</head>
<body>
<div id="Title_bar">
    <div id="Title_bar_Head">
        ...
        <div id="Title"><!--页面标题-->
            <img border="0" width="13" height="13" src="${pageContext.request.contextPath}/style/images/title_arrow.gif"/> 垃圾箱
        </div>
...
<div id="MainArea">
    <table cellspacing="0" cellpadding="0" class="TableStyle">
        ...
        <!--显示数据列表   class="dataContainer"  datakey="messageList"-->
        <tbody id="TableData">
        <c:forEach items="${email }" var="email">
        	 <tr  align=center class="TableDetail1 template">
                ...
                <td><a href="email_restore.action?eid=${email.eid }">还原</a>
					<a onClick="return delConfirm()" href="email_delete.action?eid=${email.eid }&box=waste">永久删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
...
<div id=PageSelectorBar>
	...
	<div id=PageSelectorSelectorArea>
		...
		-->
		<a href="email_box.action?uid=${user.uid}&startnum=1&box=waste" title="首页" style="cursor: hand;">
...
<%--          ${vs.count}----索引从1开始 --%>
		<c:forEach  begin="1" end="${sumnum }"  var="v" varStatus="vs">
        	<c:if test="${num != vs.count}"><span class="PageSelectorNum" style="cursor: hand;" onClick="gotoPageNum(${vs.count},${user.uid});">${vs.count}</span></c:if>
		...
		-->
		<a href="email_box.action?uid=${user.uid}&startnum=${sumnum }&box=waste" title="尾页" style="cursor: hand;"><img src="${pageContext.request.contextPath}/style/blue/images/pageSelector/lastPage.png"/></a>
